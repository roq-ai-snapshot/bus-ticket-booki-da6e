...
import { useRoqClient, useBusFindManyWithCount } from 'lib/roq';
import { convertQueryToPrismaUtil } from 'lib/utils';

type ColumnType = ColumnDef<BusInterface, unknown>;
...
export function BusListPage(props: BusListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const queryParams = useMemo(
    () =>
      convertQueryToPrismaUtil(
        {
          relations: ['company', 'route.count'],
          searchTerm: params.searchTerm,
          searchTermKeys: ['bus_number.contains', 'bus_model.contains', 'bus_brand.contains'],
          ...params.filters,
        },
        'bus',
      ),
  ...
  const { data, error, isLoading, mutate } = useBusFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.bus.delete({ where: { id } });
      await mutate();
    } catch (error) {
  ...
  const handleView = (row: BusInterface) => {
    if (hasAccess('bus', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/buses/view/${row.id}`);
    }
  };

  const columns: ColumnType[] = [
    { id: 'bus_number', header: 'Bus Number', accessorKey: 'bus_number' },
    { id: 'capacity', header: 'capacity', accessorKey: 'capacity' },
    { id: 'bus_model', header: 'Bus Model', accessorKey: 'bus_model' },
    { id: 'bus_brand', header: 'Bus Brand', accessorKey: 'bus_brand' },
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/buses/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('bus', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/buses/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('bus', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="bus" />
        </Flex>

        {hasAccess('bus', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/buses/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
