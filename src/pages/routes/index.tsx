...
import { useRoqClient, useRouteFindManyWithCount } from 'lib/roq';
...
export function RouteListPage(props: RouteListPageProps) {
  ...
  const queryParams = useMemo(
    () =>
      convertQueryToPrismaUtil(
        {
          relations: ['bus', 'booking.count'],
          searchTerm: params.searchTerm,
          searchTermKeys: ['start_point.contains', 'end_point.contains'],
          ...params.filters,
        },
        'route',
      ),
  ...
  const { data, error, isLoading, mutate } = useRouteFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.route.delete({ where: { id } });
      await mutate();
    } catch (error) {
  ...
  const columns: ColumnType[] = [
    { id: 'start_point', header: 'Start Point', accessorKey: 'start_point' },
    { id: 'end_point', header: 'End Point', accessorKey: 'end_point' },
    {
      id: 'departure_time',
    ...
    hasAccess('bus', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'bus',
          header: 'Bus',
          accessorKey: 'bus',
          cell: ({ row: { original: record } }: any) => (
            <Link as={NextLink} onClick={(e) => e.stopPropagation()} href={`/buses/view/${record.bus?.id}`}>
