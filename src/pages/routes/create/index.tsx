...
function RouteCreatePage() {
  ...
  const handleSubmit = async (values: RouteInterface, { resetForm }: FormikHelpers<any>) => {
    setError(null);
    try {
      await roqClient.route.create({ data: values as RoqTypes.route });
      resetForm();
      router.push('/routes');
    } catch (error) {
      setError(error);
  ...
  const formik = useFormik<RouteInterface>({
    initialValues: {
      ...
      bus_id: (router.query.bus_id as string) ?? null,
    },
    ...
    >
      <Box rounded="md">
        ...
        <FormWrapper onSubmit={formik.handleSubmit}>
          <TextInput
            error={formik.errors.start_point}
            label={'Start Point'}
            props={{
              name: 'start_point',
          ...
          <AsyncSelect<BusInterface>
            formik={formik}
            name={'bus_id'}
            label={'Select Bus'}
            placeholder={'Select Bus'}
            fetcher={() => roqClient.bus.findManyWithCount({})}
            labelField={'bus_number'}
          />
