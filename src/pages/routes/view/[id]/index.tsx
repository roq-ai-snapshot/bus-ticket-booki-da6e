...
import { useRouteFindFirst, useRoqClient } from 'lib/roq';
...
function RouteViewPage() {
  ...
  const queryParams = useMemo<{ include: { bus: boolean } }>(
    () =>
        ...
        {isLoading ? (
          ...
          <>
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              ...
              >
                <FormListItem label="Start Point" text={data?.start_point} />

                <FormListItem label="End Point" text={data?.end_point} />
                ...
                {hasAccess('bus', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                  <FormListItem
                    label="Bus"
                    text={
                      <Link as={NextLink} href={`/buses/view/${data?.bus?.id}`}>
                        {data?.bus?.bus_number + ''}
                      </Link>
                    }
                  />
